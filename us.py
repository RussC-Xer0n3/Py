...
'''
Additional code from UltrasonicRanging MODDED - from trig to tx and rx to differentiate I/O from Datasheets.
'''
txPin=16
rxPin=18

'''
220 define the maximum measuring distance, unit: cm 
(currently the cubic area of the self built faraday 
cage without subtraction of it's internal contents)
'''

MAX_DISTANCE=10.5*23*25.5
...
# Window shabizzle #
####################
imaged = ultrasound(distance,pulseTime,localOscilator)
frame=Frame(window)
image=Image(frame,image=imaged)
image.pack(padx=10, pady=10, side=TOP)
...
start_therm=Button(window,text='Start Thermon',command=thermostat)
start_therm.pack(side=RIGHT,padx=10)

stop_rfid=Button(window,text='Stop Thermon',command=thermostat(KeyboardInterrupt))
...
stop_ultra=Button(window,text='Stop ultrasound',command=__main__(KeyboardInterrupt))
stop_ultra.pack(side=RIGHT,padx=10)

start_rfid=Button(window,text='Start RFID',command=localOscilator)
start_rfid.pack(side=RIGHT,padx=10)

stop_rfid=Button(window,text='Stop RFID',command=localOscilator(KeyboardInterrupt))
stop_rfid.pack(side=RIGHT,padx=10)
...
The code here is the item we are trying to 'look at' whilst it is operational 
inside a faraday cage.

code will be modified regularly during experimentation and information gathering
in addition to the operation of the HC-SR04 module since further investigation 
into the frequency ranges (safe levels and amplification) will hopefully yield
visual results into the interface.

should return the frequency and or E.o.D (end-point of distribution from output
module RFID-RC522)
'''
...
'''
thermostat()
function to start the thermostat and report to the interface the temperature
inside the faraday cage subtracted from an ambient safe temperature 35.7C
such that temperature reported by the thermistor can be subtracted and the
reading values compared and prioritised for safety alarms.
'''
'''
def thermostat(therm):
	try:
		therm
		# TODO: write out to gui
		return
	except KeyboardInterrupt:
		GPIO.cleanup()
	finally:
...
def image_index(values):
	i = 79999 #central 'pixel' of image
...
we ant to be able to get an image of what we want to see
...
'''
ultrasound()
designed to modify the capabilities of the HC-SR04 module to produce ultrasound as opposed
to sonar
...
def ultrasound(distance,pulseTime,localOscilator):
	
	RF = pulseIn(rxPin,GPIO.HIGH,timeOut)
	LO = ((distance / pulseTime) - localOscilator)
	fImage = (RF - LO) - MAX_DISTANCE
	shade_index(fImage) #fimage
...
def getSonar():
	...
	distance = pingTime * 343.0 / MAX_DISTANCE / 10000.0 # calculate distance with sound speed 340m/s 
	return distance
